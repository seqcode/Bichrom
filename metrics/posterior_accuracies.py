...
def plot_beta_uniform_prior(confusion_sequence, confusion_seq_chrom, row, nrows, factor):
    """
    ...
    plt.plot(np.repeat(mode, 100), np.linspace(0, 100, 100), lw=3, color='#F1C40F')
    plt.plot(x, beta.pdf(x, 1 + tp, 1 + fn), color='grey', lw=1)

    # Sequence + chromatin network
    mode, tp, fn = get_mode(confusion_seq_chrom)
    plt.plot(x, beta.pdf(x, 1 + tp, 1 + fn), color='grey', lw=1)
    plt.plot(np.repeat(mode, 100), np.linspace(0, 100, 100), lw=3, color='grey')
...
def plot_figures(data_path):
    ...
    for idx, factor in enumerate(factor_list):
        ...
        plot_beta_uniform_prior(conf_mat_seq, conf_seq_chrom, idx, num_rows, factor)
